...
text = []
x = input("Enter the name of the .txt file")
text = find_continents(read_pokedata(x))
number = []
for i in range(len(text)):
    number = pokemon_in_continent(read_pokedata(x), text[i])
    print(text[i], ": ", str(len(number)), " total", "\n", count_types(read_pokedata(x),pokemon_in_continent(read_pokedata(x), text[i])))
